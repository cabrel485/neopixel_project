...
proc plnx_gen_conf_serial {mapping kconfprefix cpuname cpuslaves} {
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if { "$current_arch" == "aarch64"} {
				set tmp_uart [string tolower ${name} ]
				if { "${tmp_uart}" == "psu_uart_0"} {
					set atf_console "cadence"
				} elseif { "${tmp_uart}" == "psu_uart_1"} {
					set atf_console "cadence1" }
				if {[llength ${atf_console}] > 0} {
					set serialipname [format "%s\n%s\n%s\n%s\n%s\n" "${serialipname}"\
						"config ${serialkconfprefix}ATF_IP_NAME"\
						"string"\
						"default [string tolower ${atf_console}]"\
						"depends on ${serialkconfprefix}${kname}_SELECT"]
				}
			}
			if { "${baseaddr_property}" != "" } {
	...
	if { "$current_arch" == "aarch64"} {
		set serialipname [format "%s\n%s\n%s\n%s\n%s\n" "${serialipname}"\
			...
			"string"\
			"default dcc"\
			"depends on ${serialkconfprefix}MANUAL_SELECT"]
	}
